@props(['id', 'image', 'type', 'ville', 'prix', 'note', 'titre'])
...
<a href="{{ route('annonces.show', $id) }}" class="group block cursor-pointer">
    ...
    <div class="aspect-[4/3] w-full overflow-hidden rounded-xl bg-gray-200 mb-4 relative">
        <img src="{{ $image }}" alt="{{ $titre }} {{ $type }} à {{ $ville }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105">
    ...
    <div class="flex justify-between items-start">
        <div>
            <h3 class="font-semibold text-slate-900 text-[15px] leading-tight truncate max-w-[220px]">{{ $titre }}</h3>
            <p class="text-slate-500 text-sm mt-1 whitespace-nowrap overflow-hidden text-ellipsis max-w-[200px]">{{ $ville }} · {{ $type }}</p>
